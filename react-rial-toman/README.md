...
Iran's currency is officially the **Rial (IRR)**, but everyday prices are almost always spoken and written in **Toman** (1 Toman = 10 Rials, traditionally — see [A note on the conversion ratio](#a-note-on-the-conversion-ratio) below). This package gives you two small, composable React components plus the plain-JS utilities behind them, so you can:
...
| `ratio` | `number` | `10` | Rial-per-Toman ratio. See [note below](#a-note-on-the-conversion-ratio). |
